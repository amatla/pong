...
class Model {
  constructor(
    ...
    this.player = new Player(
      0 + playerPadding,
      (tableHeight - paddleHeight) / 2,
      paddleWidth,
      paddleHeight,
      'Player',
    );
    this.ai = new Player(
      tableWidth - paddleWidth - playerPadding,
      (tableHeight - paddleHeight) / 2,
      paddleWidth,
      paddleHeight,
      'AI',
  ...
  resetBall() {
    ...
    [this.ball.velocity.x, this.ball.velocity.y] = [
      Math.cos(direction) * this.initialSpeed,
      Math.sin(direction) * this.initialSpeed,
    ];
  ...
  collision() {
    let currentPlayer =
      this.ball.position.x > this.table.width / 2
        ...
        : this.player;
    if (
      currentPlayer.left < this.ball.right &&
      currentPlayer.right > this.ball.left &&
      currentPlayer.top < this.ball.bottom &&
      currentPlayer.bottom > this.ball.top
    ) {
      let collisionPt =
        this.ball.position.y -
